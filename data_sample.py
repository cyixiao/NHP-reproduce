...
dataset = "iAF1260b"
use_candidates = True
print("The data set is processing is:", dataset)
...
if 'V' in graph:
    print("Total number of vertices:", len(graph['V']))
    print("Sample vertices:", list(graph['V'])[:3])
if 'E' in graph:
    print("Total number of hyperedges:", len(graph['E']))
    print("Sample hyperedges:", list(graph['E'].items())[:3])
...
print("========== cleaning {}_hypergraph.pkl edges ==========".format(dataset))
hyperedges, edges_num, vertices_num = remove_empty_hyperedges(graph)
print(f"Number of vertices: {vertices_num}")
print(f"Number of non-empty hyperedges: {edges_num}")
print("Filtered hyperedges:", list(hyperedges.items())[:3])
...
for k in candidates['E']:
    ...
    if all((neg_tail, neg_head) != (gt, gh) for _, (gt, gh) in graph['E'].items()):
        ...
        candi_count += 1
print("========== cleaning {}_candidates.pkl edges ==========".format(dataset))
print("Number of candidate edges is", len(candidates_edges))
print("Filtered candidate edges:", list(candidates_edges.items())[:3])

# Sampling method: |e|/2 sampled from e and the remaining half from V - e
print("========== generate negative hyperlinks ==========")
negative_hyperlinks = {}
...
for i, (tail, head) in hyperedges.items():
    ...
    while True:
        ...
        if ((tuple(new_tail_set), tuple(new_head_set)) not in existing_tail_head and
            ...
            break
print("Generated negative hyperlinks:", list(negative_hyperlinks.items())[:3])

# replace sampled negative hyperlinks with candidates hyperlinks
print("========== replace negative hyperlinks with candidates ==========")
if use_candidates:
    assigned = set()
...
print("Replaced negative hyperlinks:", list(negative_hyperlinks.items())[:3])

# split train / test datasets
print("========== split train / test datasets ==========")
...
print("Sample undirected positive hyperlinks:", list(undirected_hyperedges.items())[:3])
undirected_negative = convert_to_undirected(negative_hyperlinks)
print("Sample undirected negative hyperlinks:", list(undirected_negative.items())[:3])
...
print("========== save adjacency matrix and indices ==========")
...
for name, (data, indices) in datasets.items():
    ...
    print(f"{name} matrix saved to {save_path} with shape {adjacency_matrix.shape}")
...
with open(indices_file_path, 'wb') as file:
    ...
    print(f"Indices and parameters saved to {indices_file_path}")
...
with open(file_path, 'rb') as file:
    ...
    print("Sample of train_hyperedges_i: {} with length {}"
          .format(data["train_hyperedges_i"][:3], len(data["train_hyperedges_i"])))
    print("Sample of train_negative_i: {} with length {}"
          .format(data["train_negative_i"][:3], len(data["train_negative_i"])))
